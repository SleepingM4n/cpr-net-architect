...
6. **NET combat requires GM decisions.** The module records native rolls and applies final damage chosen by the GM to encounter REZ or real runner HP/Program REZ. Opposed outcomes, mitigation, effects, critical bonuses, initiative and pursuit remain manual. ICE stats/Programs are encounter copies; source Actor effects are not automatically folded into snapshot rolls. Missing damage Programs produce an error. Native physical-token chat damage controls do not target virtual ICE; use the NET Combat confirmation button.
...
12. **Host installation depends on custom-module support.** Install through the GitHub release manifest or extract the release ZIP on your host. The host’s absolute data path cannot be inferred from the supplied ZIP.
