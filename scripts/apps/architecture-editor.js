...
import { ID, TYPES, ABILITIES, CONTROL_TYPES, clone, uid, escapeHTML as e, requireGM, assert, log, optionalDocument, sameData } from "../constants.js";
...
export class ArchitectureEditor extends NETApplication {
  ...
  activateListeners(html) {
    ...
    this.root.querySelector(".neta-node-form")?.addEventListener("input", () => {
      this.readNode();
      this.dirty = true;
    });
  }
  readNode() {
    const form = this.root?.querySelector(".neta-node-form"),
      n = this.architecture.nodes.find(n => n.id === (form?.dataset.nodeId ?? this.selected));
    if (!form || !n) return;
    const before = clone(n);
    ...
    if (!sameData(before, n)) this.dirty = true;
  }
  async saveArchitecture() {
    if (this.saving) return this.saving;
    this.readNode();
    const snapshot = clone(this.architecture);
    this.dirty = true;
    this.saveError = null;
    this.saving = Promise.resolve().then(async () => {
      try {
        const saved = await this.runtime.store.save(snapshot);
        this.readNode();
        if (sameData(this.architecture, snapshot)) {
          this.architecture = saved;
          this.dirty = false;
          ui.notifications.info("Architecture saved and verified.");
        } else {
          ui.notifications.info("Earlier changes saved. Your newer edits are still unsaved; press Save Architecture again.");
        }
        this.runtime.manager?.render(false, { focus: false });
      } catch (error) {
        this.readNode();
        this.dirty = true;
        this.saveError = `Save failed: ${error.message}. Your edits remain in this editor. Retry Save Architecture.`;
        ui.notifications.error(this.saveError);
      } finally {
        this.saving = null;
        // Never replace or rerender the form during a pending request: typing must stay intact.
